...
class Contract extends skydapp_common_1.EventContainer {
    ...
    async runMethodNew(methodName, ...params) {
        const from = await Wallet_1.default.loadAddress();
        await ExtWallet_1.default.caver.klay.sendTransaction({
            type: "SMART_CONTRACT_EXECUTION",
            from: from,
            to: this.address,
            data: Klaytn_1.default.caver.abi.encodeFunctionCall(this.findMethodABI(methodName), params),
            gas: 1500000,
        });
    }
    ...
    async runWalletMethod2(methodName, ...params) {
